...
import { addInsight } from './insights'
import type { Suggestion, Rejection } from '@/types'
import type { ScanResult } from '@/ai/scan'
import type { InsightResult } from '@/ai/surfaceInsights'
...
async function recordRejection(s: Suggestion): Promise<void> {
  ...
  if (s.kind === 'assignment' && s.itemId != null && s.themeId != null) {
    ...
    const dup = await db.rejections
      ...
      .filter((r: Rejection) => r.kind === 'proposal')
      .count()
    if (dup === 0) {
      await db.rejections.add({
        kind: 'proposal',
  ...
  } else if (s.kind === 'insight-proposal' && s.proposedHeadline) {
    const nameLower = s.proposedHeadline.trim().toLowerCase()
    const dup = await db.rejections
      .where('proposedNameLower').equals(nameLower)
      .filter((r: Rejection) => r.kind === 'insight-proposal')
      .count()
    if (dup === 0) {
      await db.rejections.add({
        kind: 'insight-proposal',
        proposedNameLower: nameLower,
        createdAt: now,
      })
    }
...
// ---------- Insight suggestion queue ----------

export async function dismissAllInsights(): Promise<void> {
  await db.suggestions
    .filter((s: Suggestion) => s.kind === 'insight-proposal')
    .delete()
}

export async function replaceInsightQueue(result: InsightResult): Promise<string> {
  const scanId = `insight-${Date.now()}`
  const now = Date.now()
  ...
  await db.transaction('rw', db.suggestions, async () => {
    ...
    for (const p of result.proposals) {
      rows.push({
        ...
        scanId,
        createdAt: now,
        proposedHeadline: p.headline,
        proposedRationale: p.rationale,
        supportingThemeIds: p.themeIds,
        supportingItemIds: p.itemIds,
        strength: p.strength,
      })
    }
  ...
  await db.settings.put({ key: 'lastInsightAt', value: String(now) })
  return scanId
}

export async function approveInsightProposal(
  id: number,
  changes: { headline?: string } = {}
): Promise<number | null> {
  const s = await db.suggestions.get(id)
  if (!s || s.kind !== 'insight-proposal') return null

  const headline = (changes.headline ?? s.proposedHeadline ?? '').trim()
  if (!headline) return null

  const insightId = await addInsight({
    headline,
    rationale: s.proposedRationale ?? '',
    themeIds: s.supportingThemeIds ?? [],
    itemIds: s.supportingItemIds ?? [],
...
export async function updateInsightProposal(
  id: number,
  changes: { headline?: string }
): Promise<void> {
  const patch: Partial<Suggestion> = {}
  if (changes.headline !== undefined) patch.proposedHeadline = changes.headline
  await db.suggestions.update(id, patch)
}

export async function getInsightRejections(): Promise<{
  proposedNamesLower: string[]
}> {
  const all = await db.rejections.toArray()
  return {
    proposedNamesLower: all
      .filter((r: Rejection) => r.kind === 'insight-proposal' && !!r.proposedNameLower)
      .map((r: Rejection) => r.proposedNameLower!),
  }
}
