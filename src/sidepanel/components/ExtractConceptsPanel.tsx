...
import { extractConcepts, ExtractError } from '@/ai/extractConcepts'
import { replaceConceptQueue } from '@/db/suggestions'
import type { ResearchItem } from '@/types'

type Status = 'idle' | 'confirming' | 'extracting' | 'error'
type Scope = 'untagged' | 'all'

export function ExtractConceptsPanel() {
  ...
  const conceptCount = useLiveQuery(() => db.concepts.count(), [])
  const apiKeyRow = useLiveQuery(() => db.settings.get('anthropicApiKey'), [])
  const lastRow = useLiveQuery(() => db.settings.get('lastConceptExtractAt'), [])

  const untaggedCount = useLiveQuery(
    () => db.items.filter((i: ResearchItem) => (i.conceptIds?.length ?? 0) === 0).count(),
    []
  )
  ...
  const canRun = hasKey && hasItems && status !== 'extracting'
  ...
  async function run() {
    ...
    try {
      ...
      const items = scope === 'untagged'
        ? await db.items
            .filter((i: ResearchItem) => (i.conceptIds?.length ?? 0) === 0)
            .toArray()
        : await db.items.toArray()
      const result = await extractConcepts(items, concepts)
      await replaceConceptQueue(result)
      setStatus('idle')
    } catch (err) {
      const msg = err instanceof ExtractError ? err.message
  ...
  let label: string
  if (!hasKey) label = 'Set API key in Settings to extract'
  else if ((itemCount ?? 0) === 0) label = 'Add items first'
  else if (status === 'extracting') label = 'Extracting…'
  else if (!hasItems) label = 'Every item has concepts'
  else if (scope === 'untagged') label = `Extract · ${effectiveCount} item${effectiveCount !== 1 ? 's' : ''}`
  else label = `Extract all · ${effectiveCount} item${effectiveCount !== 1 ? 's' : ''}`

  const approxTokens = Math.ceil((effectiveCount * 350 + (conceptCount ?? 0) * 80 + 1800) / 3.5)
  ...
  return (
    <div className="px-3 py-2 border-b border-line relative">
      <div className="flex items-center justify-between gap-2">
        ...
        >
          <span className={status === 'extracting' ? 'animate-spin' : ''}>
            {status === 'extracting' ? '⟳' : '◆'}
          </span>
          <span className="text-ink-2">{label}</span>
        </button>
        {lastAt != null && status !== 'extracting' && (
      ...
      {hasKey && (itemCount ?? 0) > 0 && status !== 'extracting' && (
        <div className="flex items-center gap-1 mt-1.5 ml-1">
          <ScopeChip
            label={`Untagged${canPickUntagged ? ` (${untaggedCount})` : ''}`}
            active={scope === 'untagged'}
            disabled={!canPickUntagged}
            title={canPickUntagged ? 'Only items without any concepts' : 'No items without concepts'}
      ...
      {status === 'confirming' && canRun && (
        ...
        >
          <p className="text-sm text-ink mb-1">
            Send {effectiveCount} item{effectiveCount !== 1 ? 's' : ''} to Claude to extract concepts?
          </p>
